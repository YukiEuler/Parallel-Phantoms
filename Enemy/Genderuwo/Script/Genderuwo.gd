...
func _physics_process(delta):
	if player_body:
		...
		if warn_time == 200:
			...
			add_child(warn_att)
		if warn_time > 195:
			sudut = atan2(velocity.x, velocity.z)
			print(delta)
			model_rotation(warn_att, delta)
	else:
		if tick == 100:
	...
	if 95 < warn_time and warn_time < 100 and player_body:
		if velocity.x != 0.0 or velocity.z != 0.0 and player.off_menu:
			att.rotation.y = lerp_angle(att.rotation.y, sudut, delta * 100)
